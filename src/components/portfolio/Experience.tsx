...
export function Experience() {
  ...
  return (
    <section id="experience" className="relative overflow-hidden px-6 pt-36 sm:pt-48 pb-24">
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute -left-40 top-1/4 h-[520px] w-[520px] rounded-full blur-3xl opacity-60 -z-10"
        style={{
          background:
            ...
            {experience.map((e, i) => {
              ...
              return (
                <div
                  ...
                  className="timeline-item relative grid grid-cols-2 gap-12 md:gap-16"
                >
                  ...
                  <div className="absolute left-1/2 top-[34px] flex h-10 w-10 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-background border border-border shadow-sm text-foreground transition-colors duration-500">
                    ...
                    <Icon
                      className={`h-4 w-4 relative z-10 transition-colors duration-500 ${activeIndex === i ? "text-primary" : "text-muted-foreground"}`}
                    />
                  </div>
                  ...
                  {left ? (
                    <>
                      <div
                        className={`relative overflow-hidden glass rounded-2xl p-6 text-right border-[0.5px] border-border/20 shadow-none transition-all duration-500 ${activeIndex === i ? "border-primary/30" : ""}`}
                      >
                        <div
                          className={`pointer-events-none absolute inset-y-0 right-0 w-32 bg-gradient-to-l from-primary/15 to-transparent transition-opacity duration-500 ${activeIndex === i ? "opacity-100" : "opacity-0"}`}
                        />
                        <div className="relative z-10">
                          <div
                            className={`transition-all duration-500 ease-in-out overflow-hidden ${activeIndex === i ? "max-h-10 opacity-100 mb-2" : "max-h-0 opacity-0 mb-0"}`}
                          >
                            <p className="text-[10px] uppercase tracking-widest text-muted-foreground/60">
                              {e.period}
                            </p>
                          </div>
                          <h3 className="font-display text-base font-semibold text-foreground hidden md:block">
                            {e.role}
                          </h3>
                          <h3 className="font-display text-base font-semibold text-foreground md:hidden">
                            {e.roleMobile || e.role}
                          </h3>
                    ...
                    <>
                      <div />
                      <div
                        className={`relative overflow-hidden glass rounded-2xl p-6 text-left border-[0.5px] border-border/20 shadow-none transition-all duration-500 ${activeIndex === i ? "border-primary/30" : ""}`}
                      >
                        <div
                          className={`pointer-events-none absolute inset-y-0 left-0 w-32 bg-gradient-to-r from-primary/15 to-transparent transition-opacity duration-500 ${activeIndex === i ? "opacity-100" : "opacity-0"}`}
                        />
                        <div className="relative z-10">
                          <div
                            className={`transition-all duration-500 ease-in-out overflow-hidden ${activeIndex === i ? "max-h-10 opacity-100 mb-2" : "max-h-0 opacity-0 mb-0"}`}
