...
export const PufferFish: React.FC<PufferFishProps> = ({
  ...
  // Puff animation & release tracking
  ...
  const handleClick = useCallback(() => {
    ...
    onFishClick();
    setTimeout(() => { puffCooldown.current = false; }, 900);
  }, [onFishClick]);
  ...
  useFrame((state, delta) => {
    ...
    /* 2 ── dynamic scale: shrinks a lot immediately before reaching corner ── */
    ...
    // Rapid scale reduction so it shrinks way before arriving at corner
    ...
    if (animState.isSwimmingDown) {
      ...
      /* 4 ── ACTIVE SWIMMING TO CORNER: dynamic dive angle */
